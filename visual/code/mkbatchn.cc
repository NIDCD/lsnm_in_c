...
/*mkbatchn.cc  rev. 8/98
random noise input for right brain and left brain
...
similarly, lgns_in, and lgnseed is for right brain
...
int main()
{
  ...
  int nsubj,cleanup,num,m,k,i,attni,ggns_in,lgns_in,ggnseed,lgnseed;
  ...
  strcpy(netgen,bin);
  strcpy(Infile,BASE);
  strcat(Infile,"code/mkbatchn.in");
  ...
  outfile = fopen("batchn","w");
  appfile = fopen("appn","w");
  ...
  fscanf(infile,"%d %d %d %d %d %d %d %f %f %f %d %d %d",
	 &nsubj,&cleanup,&m,&ggns_in,&ggnseed,&lgns_in,&lgnseed,
	 &lefthi,&leftlo,&rightlo,&start,&end,&increment);
  ...
  fprintf(outfile,
    "cat weights/right/right.s weights/left/left.sn code/appn > pethi.s\n");
  fprintf(outfile,"rm code/appn\nrm code/batchn\n");
  fprintf(outfile,"#\n");
  ...
  fprintf(outfile,"rm ggn*.inp\n");
  fprintf(outfile,"cd %s\n",BASE);
  fprintf(outfile,"rm sfiles/*attn*.s\n");
  fprintf(outfile,"mv noisy %s/nRnL\n",whereto);
  fclose(infile);
  fclose(outfile);
}
