  ...
  for(i=1; i<=n_pairs; i++)
    {
      ...
      fprintf(appfile,"#include %s/inputs/pethi%d.rsb\n",BASE,i);
      fclose(appfile);
    }

  outfile = fopen("batchb","w");
  ...
  for(i=1; i<=n_pairs; i++)
    {
      ...
      fprintf(outfile,"cat weights/right/right.s weights/left/left.sn code/appb%d > pethi%d.s\n",i,i);
      fprintf(outfile,"rm code/appb%d\n",i);
    }

  fprintf(outfile,"rm code/batchb\n");
